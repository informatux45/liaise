...
class XoopsGTicket {
	...
	function getTicketHtml( $salt = 'XoopsIsAnIncredibleCmsDontYouThinkH' , $timeout = 1800 , $area = '' )
	{
		return '<input type="hidden" name="XOOPS_G_TICKET" value="'.$this->issue( $salt , $timeout , $area ).'" />' ;
	}

	// returns an object of XoopsFormHidden including theh ticket
	function getTicketXoopsForm( $salt = 'XoopsIsAnIncredibleCmsDontYouThinkG' , $timeout = 1800 , $area = '' )
	{
		return new XoopsFormHidden( 'XOOPS_G_TICKET' , $this->issue( $salt , $timeout , $area ) ) ;
	}

	// add a ticket as Hidden Element into XoopsForm
	function addTicketXoopsFormElement( &$form , $salt = 'XoopsIsAnIncredibleCmsDontYouThinkM' , $timeout = 1800 , $area = '' )
	{
		$form->addElement( new XoopsFormHidden( 'XOOPS_G_TICKET' , $this->issue( $salt , $timeout , $area ) ) ) ;
	}

	// returns an array for xoops_confirm() ;
	function getTicketArray( $salt = 'XoopsIsAnIncredibleCmsDontYouThinkB' , $timeout = 1800 , $area = '' )
	{
		return array( 'XOOPS_G_TICKET' => $this->issue( $salt , $timeout , $area ) ) ;
	}

	// return GET parameter string.
	function getTicketParamString( $salt = 'XoopsIsAnIncredibleCmsDontYouThinkC' , $noamp = false , $timeout=1800 , $area = '' )
	{
	    return ( $noamp ? '' : '&amp;' ) . 'XOOPS_G_TICKET=' . $this->issue( $salt, $timeout , $area ) ;
	}

	// issue a ticket
	function issue( $salt = 'XoopsIsAnIncredibleCmsDontYouThink' , $timeout = 1800 , $area = '' )
	{
		global $xoopsModule ;
